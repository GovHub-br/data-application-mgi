...
### 1.2 Parâmetros de Data (Data Interval)

DAGs com filtro de data usam o **Data Interval** do Airflow (`data_interval_start` / `data_interval_end`). Para execuções agendadas, o Airflow preenche esses valores automaticamente com base no cron. Para execuções manuais com intervalo customizado, usar **Trigger DAG → Specify Manually → Start / End** na interface.

DAGs que usam este parâmetro: `contratacoes_dag`, `itens_contratacoes_dag`, `resultado_itens_contratacoes_dag`, `contratos_dag`, `contratos_item_dag`, `arp_dag`, `arp_item_dag`, `arp_detalhes_dag`.
...
| # | Endpoint | Parâmetros obrigatórios | Tipo de carga | DAG | PK |
|---|----------|-------------------------|---------------|-----|----|
| 1 | `1_consultarARP` | `dataVigenciaInicialMin`, `dataVigenciaInicialMax` | Incremental por período | `arp_dag` | `numeroataregistropreco + idcompra` |
| 2 | `2_consultarARPItem` | `dataVigenciaInicialMin`, `dataVigenciaInicialMax` | Incremental por período | `arp_item_dag` | `numeroataregistropreco + codigounidadegerenciadora + numeroitem + idcompra + nifornecedor` |
...
**Implementação:** Endpoints 1 e 2 usam `get_page_starts` + `fetch_block.expand()` em blocos de 15 páginas + `validate`. Registros com qualquer campo da PK nulo são descartados antes do insert (13.553 registros identificados na primeira carga — dados incompletos retornados pela API). Endpoints 3, 4 e 5 em `arp_detalhes_dag`: lê **todos** os itens/pares de `raw_arp_item` (sem filtro de data), agrupa em blocos de 10, e expande `fetch_unidades_adesoes` (endpoints 3+5) e `fetch_empenhos` (endpoint 4) em paralelo com `max_active_tis_per_dag=4`. Cada cadeia termina em `validate` independente.
